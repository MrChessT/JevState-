...
- **D-216** · Tarjetas con estructura fija (título a 2 líneas, zona a 1, datos con iconos, rasgos en una fila, pie abajo): en la rejilla todos los textos quedan alineados.
- **D-217** · Acciones de un clic del asistente (ver más, ordenar, ajustar la ficha, info de zona, hipoteca) resueltas por el código sin Jev: el usuario ya eligió exactamente qué quiere. Nuevas intenciones `calcular_hipoteca` e `info_zona` (catálogo recompilado). La hipoteca usa condiciones típicas (20 % de entrada, 30 años, 3 % fijo, 10 % de gastos) y se presenta siempre como orientativa.
- **D-218** · En la ficha, lo que no consta se agrupa en una sola línea con un único «Preguntar al agente», en lugar de repetirlo campo a campo.
- **D-219** · Componentes visuales compartidos en `src/ui/visual/` (banner con foto, estado vacío, barras de precio, rango p25–p75, barra del comparador, esqueletos de carga). Las fotos de ambiente se pintan como fondo CSS: si el CDN no responde, se ve el degradado de marca y nunca un icono de imagen rota.
- **D-220** · Todos los datos de los gráficos (medianas, rangos, «mejor valor» del comparador) salen de los inmuebles publicados; si no hay muestra suficiente (menos de 3), no se dibuja el gráfico.
- **D-221** · SEO: cada página indexable declara su canónica (el layout ya no la da: se heredaba la de la portada), hreflang con x-default, descripción con datos reales, Open Graph y tarjeta grande de X. Imágenes para compartir generadas (`/api/og/sitio` y `/api/og/inmueble/[ref]`, PNG 1200×630) porque las redes no muestran SVG. JSON-LD: RealEstateAgent + WebSite con SearchAction (portada), BreadcrumbList + ItemList (listados), BreadcrumbList + RealEstateListing (ficha), BreadcrumbList + Place (zona). Sitemap con prioridades, solo zonas con oferta e imagen de cada ficha. Los listados con filtros o sin resultados no se indexan. `siteUrl` usa el dominio de producción de Vercel si no hay NEXT_PUBLIC_SITE_URL.
- **D-222** · Accesibilidad: axe con WCAG 2.2 AA **y** buenas prácticas en todas las páginas, claro y oscuro (regiones con nombre único, orden de encabezados, foco que vuelve al botón al cerrar el asistente). Pruebas `e2e/seo.spec.ts` bloquean regresiones de SEO.
- **D-223** · Rendimiento: el catálogo para la interfaz (`src/catalog/publico.ts`) y `urlFicha` (`src/portal/urls.ts`) viven en módulos sin zod ni criptografía; Comparar y Favoritos bajan de 1,03 MB a 0,64 MB de JS. Fraunces solo con los pesos 500 y 600. `todas()` se cachea 60 s por instancia con Supabase.
- **D-224** · Asistente: peticiones relativas («más barato», «más grande») resueltas por el código sobre la ficha anterior; sin resultados, las sugerencias aflojan (subir presupuesto, quitar un filtro) en lugar de añadir; importes «a month/per month» son alquiler.
- **D-225** · Base de datos (migración 0011): índices por `slug` (ficha), por publicados recientes y por prefijo de zona; funciones `inmuebles_publicados()` (una llamada en vez de páginas de 48 en bucle), `inmuebles_por_ref()` y `fichas_por_ref()` (favoritos y comparador sin descargar la oferta) y `estadistica_zona()` en SQL con toda la muestra (antes solo los 48 primeros: error de cálculo). Tests pgTAP en 0007.
- **D-226** · Ficticios precalculados en `src/portal/ficticios.json` (compactado: sin lo que no consta ni el detalle interno del pipeline); se regeneran con `npm run ficticios:precalcular` y la CI comprueba que están al día. Antes cada arranque de servidor regeneraba los 300 (1–5 s).
- **D-227** · Metadatos y página comparten consulta con React `cache` (antes, dos consultas por listado y por ficha).
- **D-228** · Tema: control de tres botones que reaplica la preferencia al montarse (también al cambiar de idioma, que cambia el `<html>`); idioma con enlaces reales ES · EN. Páginas de error propias (`error.tsx`, `global-error.tsx`); favoritos y comparador con esqueleto, error con reintento y limpieza de referencias retiradas.
