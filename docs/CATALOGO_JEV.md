...
| Catálogo | Versión |
| --- | --- |
| Datos (campos, paquetes, adjudicación) | `2026-09-24.c9aa22b3` |
| Asistente (llamadas 1 y 2, valoración) | `2026-09-24.e7d1c9c4` |
...
| `superficie_construida` | area (m2) | noul (verify) / choice entre candidatos + `ninguno` | Is {candidate} the built area of the property, and not the usable area or the plot? | **sí**: Yes: it is the built area.<br>**no**: No: it is the usable area, the plot, a terrace or another figure. | 0.85 / 0.6 | sí | sí | sí | feed, jsonld, features_table, regex |
...
| `habitaciones` | integer | noul (verify) / choice entre candidatos + `ninguno` | Is {candidate} the number of bedrooms? | **sí**: Yes: it is the number of bedrooms.<br>**no**: No: it counts other rooms, or it is another number. | 0.85 / 0.6 | sí | sí | sí | feed, jsonld, features_table, regex |
